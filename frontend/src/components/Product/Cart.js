...
const Cart = () => {
    ...
    const removeFromCart = async (pid, color) => {
        const response = await apiUsers.removeCart(pid, color)
        if (response.success) {
    ...
    return (
        <div onClick={e => e.stopPropagation()} className='w-[400px] h-screen grid grid-rows-10 bg-gray-100 px-5'>
            ...
            <div className='row-span-2 h-full flex gap-4 flex-col'>
                <div className='flex justify-between'>
                    <span className='font-semibold uppercase'>Subtotal</span>
                    <span>{formatPrice(userData?.cart?.reduce((sum, item) => sum + item?.price * item.quantity, 0)) + ' VND'}</span>
                </div>
